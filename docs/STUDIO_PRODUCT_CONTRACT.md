...
| `result.viewer.shared` | **Recorded demo** or **Your clip** viewer | Uses one result composition for media chrome, Split/Cinema/Full screen modes, learning panel, Moments overlay, and Customize learning face. Authority-specific adapters supply media, playback, and learning projections | Keep the visible result consistent while the always-on badge and projection authority preserve recorded versus production meaning | A+B / shared presentation with separate authority adapters |
