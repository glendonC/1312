...
export function orchestratorPrompt(task: TaskRecord): string {
  ...
  const exactTools = restudied
    ? ["task_spawn_request", "task_reports_wait", "report_disposition", "artifact_read", "study_restudy_request", ...(separationEnabled ? ["study_separation_request"] : []), ...(researchEnabled ? ["study_research_request"] : []), "study_synthesize"]
    : generalized
  ...
  return [
    ...
    ...(generalized ? [
      ...
      ...(restudied ? [
        ...
        ...(researchEnabled ? ["researchInput is a durable pre-synthesis inspection result. Only its conflicting v3 coverage entries appear as triggers; an empty trigger list grants nothing. study_research_request is optional and available only by copying one exact researchInput inputId and triggerId. The host mints the research grant, fixes the gap binding, domain allowlist, budgets, and child contract. Search snippets are routing hints and never citations; receipted document spans are cite-only external context. Research never becomes claim support, caption text, transcript authority, truth, or a quality claim."] : []),
