...
export const ORCHESTRATOR_COMPUTER_USE_TOOL = "study_computer_use_request" as const;
...
export type OrchestratorToolName =
  ...
  | typeof ORCHESTRATOR_COMPUTER_USE_TOOL
  | typeof ORCHESTRATOR_SYNTHESIZE_TOOL;

export interface OrchestratorToolManifest {
  schema: "studio.orchestrator-tools.v1";
  tools: Array<{ name: OrchestratorToolName; capability: "task.spawn.request" | "task.reports.wait" | "report.disposition" | "artifact.read" | "study.plan" | "study.restudy" | "study.separate" | "study.research" | "study.computer-use" | "study.synthesize" }>;
}
...
export interface StudyRestudyToolResult {
  schema: "studio.orchestrator-study-restudy-result.v1";
  input: StudyRestudyInput;
  requestReceipt: RangePassRequestReceipt;
  requestReceiptContentId: string;
  spawn: SpawnToolResult;
}

export interface StudySynthesisToolResult {
  schema: "studio.orchestrator-study-synthesis-result.v1" | "studio.orchestrator-study-synthesis-result.v2" | "studio.orchestrator-study-synthesis-result.v3";
  studyId: string;
  artifactId: string;
  contentId: string;
  executorReceiptContentId: string;
  executorReceipt: OwnedMediaStudyExecutorReceipt | OwnedMediaStudyExecutorReceiptV2 | OwnedMediaStudyExecutorReceiptV3;
}
...
export class BoundedOrchestratorBridge {
  ...
  constructor(input: {
    ...
    computerUseRequestHost?: ComputerUseRequestExecutionHost;
    ...
    this.generalized = this.task.requiredOutputs.some((output) => output.required && (output.artifactKind === "studio.owned-media-study.v2" || output.artifactKind === "studio.owned-media-study.v3"));
  ...
  manifest(): OrchestratorToolManifest {
    ...
    const restudiedCapabilities = ["report.disposition", "artifact.read", "study.restudy", "study.synthesize"] as const;
    ...
    const computerUseEnabled = restudiedEnabled && capabilities.has("study.computer-use");
    if ([planningEnabled, generalizedEnabled, restudiedEnabled].filter(Boolean).length > 1 ||
        ...
        (computerUseEnabled !== Boolean(this.computerUseRequestHost)) ||
        ...
        (restudiedEnabled && capabilities.size !== 6 + (separationEnabled ? 1 : 0) + (researchEnabled ? 1 : 0) + (computerUseEnabled ? 1 : 0))) {
    ...
    return {
      ...
      tools: [
        ...
        ...(planningEnabled || generalizedEnabled || restudiedEnabled ? [
          ...
          ...(computerUseEnabled ? [{ name: ORCHESTRATOR_COMPUTER_USE_TOOL, capability: "study.computer-use" as const }] : []),
  ...
  async restudy(value: unknown): Promise<StudyRestudyToolResult> {
    this.manifest();
    if (!this.restudied || !this.rangePassHost) throw new OrchestratorBridgeError("capability_not_granted", "Study re-study is unavailable.");
    const callId = this.nextCallId(ORCHESTRATOR_RESTUDY_TOOL);
    await this.recordCall(callId, ORCHESTRATOR_RESTUDY_TOOL);
    let result: Awaited<ReturnType<RangePassHost["request"]>>;
    try {
      result = await this.rangePassHost.request(this.executionId, callId, value);
    } catch (error) {
      throw new OrchestratorBridgeError("operation_rejected", error instanceof Error ? error.message : "The bounded re-study request failed closed.");
    }
    if (result.decision.permit) {
      const permit = result.decision.permit;
      const launch = this.childLauncher.launch(permit).then(() => undefined).catch(async (error: unknown) => {
        if (error instanceof Error && error.name === "RuntimeApplicationInterrupted") throw error;
        const child = this.ledger.state().tasks[permit.taskId];
        if (child && (child.status === "scheduled" || child.status === "working") && child.ownerAgentId === permit.agentId) {
          await this.scheduler.transitionTask(child.id, permit.agentId, "failed", "The accepted re-study child executor failed before making a terminal report available.").catch(() => undefined);
        }
      });
      void launch.catch(() => undefined);
      this.launches.set(permit.taskId, launch);
    ...
    return {
      schema: "studio.orchestrator-study-restudy-result.v1",
      input: result.input,
      requestReceipt: result.receipt,
      requestReceiptContentId: result.receiptContentId,
      spawn: {
        schema: "studio.orchestrator-spawn-result.v1",
        requestId: result.decision.requestId,
        decision: result.decision.accepted ? "accepted" : "rejected",
        rejection: result.decision.rejection,
        followUpId: null,
      },
    };
  }

  async separation(value: unknown): Promise<{ schema: "studio.orchestrator-separation-result.v1"; input: Awaited<ReturnType<ConditionalSeparationRequestHost["inspect"]>>; spawn: SpawnToolResult }> {
    this.manifest();
    if (!this.restudied || !this.separationRequestHost) throw new OrchestratorBridgeError("capability_not_granted", "Conditional separation is unavailable.");
    const input = await this.separationRequestHost.inspect(this.executionId).catch((error) => {
      throw new OrchestratorBridgeError("operation_rejected", error instanceof Error ? error.message : "Conditional separation inspection failed closed.");
    });
    const callId = this.nextCallId(ORCHESTRATOR_SEPARATION_TOOL);
    await this.recordCall(callId, ORCHESTRATOR_SEPARATION_TOOL);
    const decision = await this.separationRequestHost.request(this.executionId, callId, value).catch((error) => {
      throw new OrchestratorBridgeError("operation_rejected", error instanceof Error ? error.message : "Conditional separation request failed closed.");
    });
    if (decision.permit) {
      const permit = decision.permit;
      const launch = this.childLauncher.launch(permit).then(() => undefined).catch(async (error: unknown) => {
        ...
        const child = this.ledger.state().tasks[permit.taskId];
        if (child && (child.status === "scheduled" || child.status === "working") && child.ownerAgentId === permit.agentId) {
          await this.scheduler.transitionTask(child.id, permit.agentId, "failed", "The accepted separation child failed before making a terminal report available.").catch(() => undefined);
        }
      });
      void launch.catch(() => undefined);
      this.launches.set(permit.taskId, launch);
  ...
  async computerUse(value: unknown): Promise<{ schema: "studio.orchestrator-computer-use-result.v1"; input: Awaited<ReturnType<ComputerUseRequestExecutionHost["inspect"]>>; spawn: SpawnToolResult }> {
    this.manifest();
    if (!this.restudied || !this.computerUseRequestHost) throw new OrchestratorBridgeError("capability_not_granted", "Cause-gated computer-use is unavailable.");
    const input = await this.computerUseRequestHost.inspect(this.executionId).catch((error) => {
      throw new OrchestratorBridgeError("operation_rejected", error instanceof Error ? error.message : "Computer-use inspection failed closed.");
    });
    const callId = this.nextCallId(ORCHESTRATOR_COMPUTER_USE_TOOL);
    await this.recordCall(callId, ORCHESTRATOR_COMPUTER_USE_TOOL);
    const decision = await this.computerUseRequestHost.request(this.executionId, callId, value).catch((error) => {
      throw new OrchestratorBridgeError("operation_rejected", error instanceof Error ? error.message : "Computer-use request failed closed.");
    });
    if (decision.permit) {
      const permit = decision.permit;
      const launch = this.childLauncher.launch(permit).then(() => undefined).catch(async (error: unknown) => {
        ...
        const child = this.ledger.state().tasks[permit.taskId];
        if (child && (child.status === "scheduled" || child.status === "working") && child.ownerAgentId === permit.agentId) {
          await this.scheduler.transitionTask(child.id, permit.agentId, "failed", "The accepted computer-use child failed before making a terminal report available.").catch(() => undefined);
        }
      });
      void launch.catch(() => undefined);
      this.launches.set(permit.taskId, launch);
    ...
    return {
      schema: "studio.orchestrator-computer-use-result.v1",
      input,
      spawn: { schema: "studio.orchestrator-spawn-result.v1", requestId: decision.requestId, decision: decision.accepted ? "accepted" : "rejected", rejection: decision.rejection, followUpId: null },
    };
  }
  ...
  async disposition(value: unknown): Promise<ReportDispositionToolResult> {
    ...
    if (this.generalized) {
      ...
      if (item.outcome === "rejected") {
        const rangePass = Object.values(this.ledger.state().rangePasses).find((entry) => entry.accepted && entry.taskId === report.taskId);
        if (rangePass && this.rangePassHost) {
          const child = this.ledger.state().tasks[report.taskId];
          if (child?.status === "working") {
            await this.scheduler.transitionTask(child.id, child.assignedAgentId, "withheld", "The exact re-study report was rejected by the parent and cannot support this range.");
          }
          await this.rangePassHost.finalizeTask(report.taskId);
        }
