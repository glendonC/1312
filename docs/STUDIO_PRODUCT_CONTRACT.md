...
| `run.production.learning-preps` | **Customize learning and Moments overlay** | For the active verified caption result, the face arms lenses and one closed temperature, posts one explicit typed prep request, and projects host-derived not-requested, loading, ready, failed, or unavailable states with closed reasons. Ready summaries show per-lens surfaced or abstained outcomes, beat or watch-through segmentation, and honest all-withheld. With verified private playback, the overlay surfaces at most one prepared available note for the moment under the playhead and is otherwise silent; available moments also mark the progress bar as waypoint dots, with a standing learning-bar count of how many exist | An unconfigured executor is non-retryable unavailable and a real provider requires explicit host-side model opt-in; revoked caption authority blocks new prep; no fixture prose appears; overlay silence is a valid state, not an error. Prep availability is receipted `not_reviewed` caption-context inference, never verified culture/history, ranking truth, or learner persistence | B / real host producer, strict client, controller, face, and overlay over verified prep artifacts |
...
| `result.recorded-learning-prep` | **Recorded learning overlay** | Opens run-006 with prepared grammar and situating notes derived only from its bound `studio.learning-prototype-fixture.v1`. Changing lenses or temperature clears the overlay until **Prepare learning for this clip** is selected | Demonstrate the shared learning result without claiming a runtime producer, receipt, semantic review, or production authority | A / recorded `design_fixture`, session-local interaction |
...
- Treat a learning-prep artifact, receipt, or Moments-overlay note as verified culture/history,
  alignment evidence, semantic quality, salience-ranking truth, course structure, a watch gate,
  learner persistence, mastery, SRS, or export eligibility; overlay silence and all-withheld prep
...
| Learning-prep producer, cold audit, strict client, controller, face, and overlay | `src/studio/runtime/production/learningPrep/`, `src/studio/runtime/production/model/learningPrep.ts`, `src/studio/localRuntime/client/learningPrepResponses.ts`, `src/studio/localRuntime/productionLearningPrepController.ts`, `src/studio/learning/LearningFineTuneFace.tsx`, `src/studio/learning/MomentsOverlay.tsx`, `tests/studio-learning-prep.test.ts` |
