...
export function assertRuntimeObservabilityIndex(
  ...
  receiptSources.forEach((entry, index) => {
    ...
    oneOf(source.kind, new Set(["media_operation", "frame_sampling", "ocr", "visual_transition", "speaker_overlap", "semantic_media_evidence", "evidence_assessment", "evidence_decision", "study_planning_decision", "study_range_pass", "owned_media_study", "study_readiness", "publish_review_intake", "publish_review_decision", "publish_review_revocation", "caption_production", "caption_quality_control", "root_output_disposition", "parent_artifact_disposition", "parent_admission", "parent_artifact_read", "executor_span", "model_usage"]), `index.sources.receipts[${index}].kind`);
