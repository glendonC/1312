...
export function assertRuntimeObservabilityIndex(
  ...
  receiptSources.forEach((entry, index) => {
    ...
    oneOf(source.kind, new Set(["media_operation", "evidence_assessment", "evidence_decision", "publish_review_intake", "executor_span", "model_usage"]), `index.sources.receipts[${index}].kind`);
