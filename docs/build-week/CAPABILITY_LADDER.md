...
| Learning fine-tune prep and moments overlay | A second private post-caption Apply producer accepts one exact verified caption identity plus a typed learner fine-tune (armed `word_order`, `grammar_salience`, `situating`, `culture_reference`, and `historical_reference` lenses with one closed temperature) and stores receipted beat-or-watch-through segmentation and per-moment candidates with closed abstentions, cold audit, authenticated GET/POST, a strict client, a request controller, a Customize learning face, and a watch-first Moments overlay that surfaces at most one prepared available note per moment and stays silent otherwise. Temperature only caps surfaced available notes. The default executor is unavailable, visibly non-retryable, and real OpenAI execution requires an explicit host-configured model and flag | Verified culture/history truth, external grounding or citations, alignment evidence, semantic quality or salience-ranking claims, automatic surfacing without an explicit prep action, learner persistence, courses, quizzes, SRS, and export | [`learningPrep/`](../../src/studio/runtime/production/learningPrep/), [`model/learningPrep.ts`](../../src/studio/runtime/production/model/learningPrep.ts), [`LearningFineTuneFace.tsx`](../../src/studio/learning/LearningFineTuneFace.tsx), [`MomentsOverlay.tsx`](../../src/studio/learning/MomentsOverlay.tsx), and [`tests/studio-learning-prep.test.ts`](../../tests/studio-learning-prep.test.ts) |
