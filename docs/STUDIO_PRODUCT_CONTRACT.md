...
- Source entry now separates **YouTube local ingest**, **Owned file local ingest**, **Explore run-006 recorded
  demo**, and the recorded YouTube preview field. The YouTube local action opens the authenticated
...
|---|---|---|---|---|
| 1. Add source | User supplies a supported link or owned clip | Source choices explicitly separate **YouTube local ingest**, **Owned file local ingest**, **Explore run-006 recorded demo**, and a recorded-preview URL field. Both local entries connect to the authenticated loopback surface; YouTube requires a bounded URL range plus local-processing confirmation, while owned files require ownership/control attestation | A+B+C / explicit recorded preview plus real private product wiring for both local authorities | Hosted/cloud ingest remains missing; both byte-ingest paths are development-only and private |
...
| 11b. Fine-tune learning and watch with a moments overlay | Optional armed lenses and temperature, one explicit prep action, per-moment prepared help, honest silence | For one verified production caption result, a Customize learning face arms an ordered subset of `word_order`, `grammar_salience`, `situating`, `culture_reference`, and `historical_reference` plus one closed low/medium/high temperature. One explicit action posts the typed `studio.learning-fine-tune.v1` request; the host stores immutable `studio.learning-prep.artifact.v1`/receipt lineage and the strict client re-hashes both before projection. A Moments overlay in a non-caption corner of the private player surfaces at most one prepared available note for the moment under the playhead and stays silent otherwise; withheld candidates, abstained lenses, and an honest all-withheld prep remain visible as closed states in the face. The default executor is visibly unavailable and non-retryable, a real provider requires explicit host-side model opt-in, and changing the fine-tune requires a new explicit prep action | B / real private producer, cold audit, strict client, controller, face, and overlay projection | Prepared notes stay `not_reviewed` caption-context inference with empty external citations; never present them as verified culture/history, alignment evidence, salience truth, a course, or a quiz gate. Temperature never coerces availability. No learner persistence, export, or Learning OS |
...
| `source.youtube-local.open` | **YouTube local ingest** | Opens the product local-runtime surface in `youtube` mode; it never creates `StudioPreviewSession` or calls replay `start()` | Client state plus `ProductLocalRuntime` | Preserve the local/recorded authority split | B / real local path entry |
