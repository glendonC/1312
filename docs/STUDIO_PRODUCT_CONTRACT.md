...
| 8. Coordinate | Children inspect scoped inputs and report structured outputs upward | Recorded prose remains replay-only; the local child can invoke scheduler-granted media, existing-evidence read, and read-receipt assessment tools through task-private bridges, retain real receipt identities and bounded claims in its structured output, and report upward. The parent decision and report remain separately validated | A+B / bounded media/evidence/assessment tools plus report-up path wired; larger coordination missing | Parent/orchestrator execution, dependencies, and retries |
| 9. Work through media | Agents seek, loop, mark, extract, inspect frames/tracks, and run detectors | Recorded workspaces remain projections. The local child exposes only scheduler-granted `media.seek`/`media.extract`, `evidence.read`, and, after completed reads, `analysis.evidence.assess`; run-005 performs one real seek, two reads, and one range/citation-bound assessment. These consume existing VAD/language facts only; they do not run detectors or return playable media | B+C / real seek, existing-evidence read, and assessment path plus bridge-tested extract; remaining tools/detectors missing | Add operations/detectors individually with grants, receipts, and honest child-visible results |
...
operator starts the loopback host and connects Studio with its bearer token
  ...
  -> an atomic production adapter projects source-artifact, task, spawn, worker, grant, operation, output-artifact, and report facts outside replay state
...
- An atomic dedicated production adapter and boring
  source-artifact/evidence-artifact/assessment-artifact/task/spawn/worker/grant/media-operation/evidence-read/evidence-assessment/output-artifact/report region; rejected batches
