...
- Source entry now separates **YouTube local ingest**, **Owned file local ingest**, **Explore run-006 recorded
  demo**, and the recorded YouTube preview field. The YouTube local UI action still stops at an
  explicit unavailable boundary and never substitutes recorded evidence. Separately, the authenticated
  loopback host now accepts an explicitly confirmed, at-most-120-second YouTube range, resolves and
  downloads it through a host-owned yt-dlp/ffmpeg seam, seals exact private bytes plus provider and
  local-processing receipts, and hot-registers the provider-neutral source session. Product wiring to
  that endpoint remains separate work.
...
|---|---|---|---|---|
| 1. Add source | User supplies a supported link or owned clip | Source choices explicitly separate a fail-closed **YouTube local ingest** entry, **Owned file local ingest**, **Explore run-006 recorded demo**, and a recorded-preview URL field. The YouTube UI entry is not wired yet; the authenticated loopback host separately accepts a confirmed bounded YouTube-local request. The owned entry connects to the loopback host, accepts browser-selected owned bytes only after explicit ownership/control attestation, and keeps CLI preflight registration as an escape hatch | A+B+C / explicit recorded preview, real browser-owned source path, real host-only YouTube-local producer, and honest unwired YouTube UI | Hosted/cloud ingest remains missing; both byte-ingest paths are development-only and private |
| 2. Validate and probe | Access, rights, duration, tracks, speech, languages, and known gaps | Owned and YouTube-local ingest preserve bytes under ignored `.studio/`, run ffprobe, seal V1 preflight, and hot-register provider-neutral sessions. YouTube-local additionally retains canonical provider identity, requested provider range, resolver/tool lineage, and an explicit no-redistribution local-processing receipt. Speech/language findings remain unavailable for either V1 path | B / real and wired at the host for both local source authorities; YouTube product wiring and detectors remain | Add detectors only through their sealed producer chain; do not infer findings from media, filename, or provider metadata |
...
| `local.source.youtube-ingest` | Canonical YouTube URL, bounded start/end, explicit local-processing confirmation | Authenticated host request resolves provider metadata, downloads at most 120 seconds through yt-dlp/ffmpeg, probes and seals the exact private bytes, and hot-registers a `youtube_local` source. It accepts no destination path and grants no redistribution/publication authority | `POST /v1/youtube-local-ingests` | Wire the existing YouTube-local product entry without creating a preview session or replay identity | B / real host path; product UI not wired |
| `local.source.youtube-progress` | **queued**, **resolving**, **downloading**, **probing**, **sealing**, **registered**, or **failed** | Polls the real producer-backed job; terminal registration returns only provider-neutral source/session identities and `sourceKind: youtube_local` | `GET /v1/youtube-local-ingests/:ingestId` | No fabricated percentages, automatic retry, recorded fallback, or path disclosure | B / real host path; product UI not wired |
