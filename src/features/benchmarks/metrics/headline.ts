import type { MetricDefinition } from "../types";

export const headlineMetrics: MetricDefinition[] = [
  {
    id: "critical-meaning",
    label: "The lines that matter",
    priority: "headline",
    question: "Did the English keep the meaning that matters?",
    layer: "End to end",
    evaluation: "human",
    requiredData: "Frozen critical units, context, independent blinded ratings, adjudication",
    support: "ready",
    field: "results[].headline.critical_meaning",
    limitation: "Binary labels simplify nuance; publish passes and denominator per clip.",
  },
  {
    id: "critical-outcomes",
    label: "Right without bluffing",
    priority: "headline",
    question: "For every line that matters, was it shown & right, shown & wrong, held back, or missed?",
    layer: "Honesty",
    evaluation: "hybrid",
    requiredData: "Gold critical units, emitted output, gate decision, human correctness label",
    support: "ready",
    field: "results[].headline.critical_outcomes",
    limitation: "The four states must stay separate; one combined score would hide the tradeoff.",
  },
  {
    id: "catastrophic-errors",
    label: "Serious errors shown",
    priority: "headline",
    question: "How often did it show flipped meaning, the wrong person or number, or an invented line?",
    layer: "Honesty",
    evaluation: "human",
    requiredData: "Frozen severity taxonomy, reviewer labels, emitted-content denominator",
    support: "ready",
    field: "results[].headline.catastrophic",
    limitation: "A low count must be read with the correct, wrong, withheld, and missing totals.",
  },
  {
    id: "latency",
    label: "Time to first caption",
    priority: "headline",
    question: "How long after ingest until captions are watchable, and until the full pack is done?",
    layer: "Speed",
    evaluation: "automated",
    requiredData: "Instrumented start, first-usable and complete events plus environment metadata",
    support: "ready",
    field: "results[].headline.latency",
    limitation: "The usability gate, hardware, model versions, network and cache state must be pinned.",
  },
];
