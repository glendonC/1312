...
export function orchestratorPrompt(task: TaskRecord): string {
  ...
  return [
    ...
    ...(requiresCoverageStudyDelegation ? [
      `This owned-study task requires at least two accepted child contracts. Every initial coverage child must use budget exactly {"wallMs":180000,"toolCalls":2}, require exactly one ${generalized ? "studio.study-report.v2" : "studio.study-report.v1"} output, and request both speech.transcribe and report.submit. The launcher verifies these model-authored contracts; a rejected request does not count. Never retry an equivalent rejected range under another workload key.`,
    ] : []),
    ...
    ...(generalized ? [
      ...
      ...(restudied ? [
        ...
        ...(researchEnabled ? ["researchInput is a durable pre-synthesis inspection result. Only its conflicting v3 coverage entries appear as triggers; an empty trigger list grants nothing. When it is empty, call study_synthesize immediately with the exact synthesisInput; do not deliberate, retry, or make another tool call. study_research_request is optional and available only by copying one exact non-empty researchInput inputId and triggerId. The host mints the research grant, fixes the gap binding, domain allowlist, budgets, and child contract. Search snippets are routing hints and never citations; receipted document spans are cite-only external context. Research never becomes claim support, caption text, transcript authority, truth, or a quality claim."] : []),
