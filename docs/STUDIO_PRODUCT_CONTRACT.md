...
- A local runtime-start host now exists under
  ...
  worker, grant, and report view-models in the owned-source status surface. This is a real and wired
  local product fragment, not a caption or swarm loop.
...
- The local scheduler, artifact store, journal, one-child launcher, forecast freeze, and structured
  ...
  poll, and project its validated task/worker/grant/report facts in a separate production region,
  but does not project them into the recorded canvas or publish results.
...
| 7. Spawn agents | Scheduler creates bounded tasks and workers | Recorded manifest/traces animate only for replay; the owned-source path projects validated scheduler tasks and registered workers through a dedicated production adapter without inserting them into replay topology | A+B / bounded local task and worker facts wired separately from replay | Add spawn-decision, operation, and richer lineage views without creating a swarm-completeness claim |
...
| 12. Inspect real run | Operator can audit actual tasks, operations, usage, handoffs, and failures | The owned-source status surface projects polled task/worker/grant/report facts; the fuller local journal remains manually loadable in `/studio/runtime/` | B / bounded product projection plus full inspector | Add operation/artifact source links to the product projection without merging protocols |
...
| Dynamic agent registry/grants/scopes | Swarm/agent focus | Production scheduler/registry | Real local; worker identities and exact grants/scopes are projected in the default owned-source surface without entering the replay graph | Architecture |
...
operator starts the loopback host and connects Studio with its bearer token
  ...
  -> an atomic production adapter projects task, worker, grant, and report facts outside replay state
...
- An atomic dedicated production adapter and boring task/worker/grant/report region; rejected
  batches preserve the last completely accepted view and never create legacy traces or a `RunBundle`
- Explicit UI labels for the production local one-child proof versus recorded demonstration

Still open: hosted/link ingest, speech/language detection for browser V1 ingest, production
spawn-decision/operation/artifact-lineage views, operation/tier choices, calibrated estimation, and
every larger-runtime item below. This slice does not produce autonomous media captions.
...
| Dedicated task/worker/grant/report adapter | `src/studio/runtime/production/studioProjection.ts` |
