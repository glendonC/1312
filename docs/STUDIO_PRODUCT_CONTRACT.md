...
| 11c. Translate selected span | Exact caption moment, selected span, one bounded span translation, availability, and limitations | The recorded demo's Translate reveals only the line's recorded translation with a "This line" tag and calls no model. On a live production caption result with valid private playback, the floating selection bar's Translate issues one explicit request that binds the exact verified caption identity and Unicode code-point span; the host stores immutable `studio.span-translation.artifact.v1`/receipt lineage over one loopback-only local Ollama executor, and the strict client re-hashes both before projection. The readout shows loading, the one host-derived translation into the opposite caption language, withheld decline, failed with bounded explicit retry, or non-retryable unavailable when no executor is configured; every missing state is visible beside the line's own tagged translation. Stale selections invalidate responses and concurrent identical requests coalesce | B+C / recorded line-scoped reveal plus real private producer, exact-span interaction, adapter, controller, and strict client | Keep this one bounded not_reviewed gloss of the exact selection. Never add fixture fallback, semantic review, dictionary claims, batch translation, learner persistence, or export without separate producers/contracts |
