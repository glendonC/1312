...
| Selected-language explanation | A separate private post-caption Apply host accepts one exact verified caption span, derives bounded context, and stores typed meaning/word/phrase/grammar/translation-choice facets with grant, artifact, receipt, cold audit, authenticated POST/GET, and strict browser parsing. Verified production captions now bind content-bound private playback and explicit exact-span facet requests through the shared learning presentation. The default executor is unavailable, visibly non-retryable, and provider/model choice is host-configured | Semantic review/correctness, grounded follow-up, listening diagnosis, culture/reference authority, learner persistence, grading, mastery, SRS, and export | [`languageExplanations/`](../../src/studio/runtime/production/languageExplanations/), [`model/languageExplanations.ts`](../../src/studio/runtime/production/model/languageExplanations.ts), [`learning/`](../../src/studio/learning/), and [`RUNTIME_CONTRACTS.md`](../RUNTIME_CONTRACTS.md) |
| Learning notes prep and tappable marks | A second private post-caption Apply producer accepts one exact verified caption identity plus a typed learner fine-tune (armed `word_order`, `grammar_salience`, `situating`, `culture_reference`, and `historical_reference` lenses with one closed temperature) and stores receipted beat-or-watch-through segmentation and per-moment candidates with closed abstentions, cold audit, authenticated GET/POST, a strict client, a request controller, a Learning notes face, and tappable per-line note marks that open prepared available notes only on demand and stay silent otherwise. Temperature only caps surfaced available notes. The default executor is unavailable, visibly non-retryable, and real OpenAI execution requires an explicit host-configured model and flag | Verified culture/history truth, external grounding or citations, alignment evidence, semantic quality or salience-ranking claims, automatic surfacing without an explicit prep action, learner persistence, courses, quizzes, SRS, and export | [`learningPrep/`](../../src/studio/runtime/production/learningPrep/), [`model/learningPrep.ts`](../../src/studio/runtime/production/model/learningPrep.ts), [`LearningFineTuneFace.tsx`](../../src/studio/learning/LearningFineTuneFace.tsx), [`momentContent.tsx`](../../src/studio/learning/momentContent.tsx), and [`tests/studio-learning-prep.test.ts`](../../tests/studio-learning-prep.test.ts) |
| Span translation in the selection bar | A third private post-caption Apply producer accepts one exact verified caption identity plus one bounded Unicode code-point span and stores one receipted `not_reviewed` translation into the opposite caption language with closed abstentions, cold audit, authenticated GET/POST, a strict client, a request controller, and a selection-bar readout whose loading, translated, withheld, failed, and unavailable states are all visible beside the line's own tagged translation. The default executor is unavailable and visibly non-retryable; real execution requires an explicit allow flag with either a host-configured local Ollama model on a loopback endpoint or an OpenAI model on the same shared prompt contract. The recorded demo offers no Translate action and calls no model | Dictionary or semantic-quality claims, verified translation correctness, automatic requests on selection, batch or whole-line generation, silent provider fallback, learner persistence, and export | [`spanTranslations/`](../../src/studio/runtime/production/spanTranslations/), [`model/spanTranslations.ts`](../../src/studio/runtime/production/model/spanTranslations.ts), [`SelectionBar.tsx`](../../src/studio/learning/SelectionBar.tsx), and [`tests/studio-span-translation.test.ts`](../../tests/studio-span-translation.test.ts) |
