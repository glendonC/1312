...
- The local scheduler, artifact store, journal, one-child launcher, forecast freeze, and structured
  ...
  default proof. Neither tool returns media bytes or semantic findings to the child. Validated
...
- `analysis.evidence.assess` is a separate opinion layer over completed evidence reads. Its
  path-free `evidence_assess` tool accepts only same-task `studio.evidence-read.receipt.v1`
...
- `analysis.evidence.decide` is the next separate honesty gate. Its path-free `evidence_decide`
  ...
  `proceed_to_publish_review` with `all_audited_claims_supported`. The latter means only that a
  future publish-review producer may consume the receipt; no caption, publication, correctness, or
  media-truth claim follows. Run-005 emits one private content-addressed
...
| 8. Coordinate | Children inspect scoped inputs and report structured outputs upward | Recorded prose remains replay-only; the local child can invoke scheduler-granted media, existing-evidence read, read-receipt assessment, and audited-assessment decision tools through task-private bridges. The deterministic decision is separately receipted before report-up; it accepts no worker prose or caller-selected outcome | A+B / bounded media/evidence/assessment/decision tools plus report-up path wired; larger coordination missing | Parent/orchestrator execution, dependencies, and retries |
...
| 11. Publish output | Timed source, translation, withheld lines, captions, study, evidence, exports | Recorded captions, comparison, scores, evidence, glossary, and artifact links render. A production `proceed_to_publish_review` decision is not shown as output and has no caption/publication consumer | A / recorded replay; production publication missing | Add a separate review producer before any caption/publish producer can consume a verified decision receipt |
| 12. Inspect real run | Operator can audit actual tasks, operations, usage, handoffs, and failures | The owned-source status surface projects source/evidence/assessment/decision artifacts, tasks, spawns, workers, grants, media operations, evidence reads/assessments/decisions, output lineage, and reports. Separate assessment-audit and decision-receipt regions appear only after the host reopens and validates stored bytes and complete lineage; the decision region retains `withheld` and stable reasons. Artifact references link only to identities rendered in that projection. The fuller local journal remains manually loadable in `/studio/runtime/` | B / bounded product projection, stored-receipt audits, plus full inspector | Expand only with producer-backed facts; do not deep-link into unrelated inspector state |
...
| Publish result | Future immutable artifact ids plus terminal/partial/withheld status; no production endpoint exists | Do not treat `proceed_to_publish_review` as captions, publication, or review completion; withheld reasons remain visible |
...
- Treat `proceed_to_publish_review` as captions existing, correct English, review completion, upload,
  or publication; it is only authority for a future review producer
