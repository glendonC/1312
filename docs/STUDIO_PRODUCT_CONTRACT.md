...
- Selected-language explanation is a separate private post-caption Apply producer. Its closed POST
  ...
  controls until content-bound private media playback is available.
...
| 11a. Explain selected language | Exact caption moment, selected span, prepared explanation facets, availability, and limitations | Recorded Results explicitly bind `studio.learning-prototype-fixture.v1` through a prototype adapter. Production caption results use the same presentation tree through a verified-caption adapter, but show `production_media_playback_unavailable` and expose no selection, My Set, or request control. A separate strict production adapter and controller map only meaning, word, phrase, grammar, and translation-choice artifacts across loading, available, partial, withheld, unavailable, failed, and explicit retry states. Default execution remains unavailable until an operator explicitly selects and enables a model. The loopback host now mints a ten-minute, exact-Origin path capability through its durable bearer, cold-revalidates the complete runtime source before every GET or HEAD, serves one native byte range with closed probe-derived MIME at source timestamp zero, and supports explicit revocation. The strict client validates the full identity and lifetime response and returns a disposable path-only handle. No React surface consumes that handle yet, so production playback and selection remain unavailable | A+B+C / explicit design-fixture mode plus real private producer, adapter, controller, unavailable production UI boundary, verified private media transport, and strict client | Bind one decoded player to the active runtime, source, caption, and source-zero timestamp authority before wiring exact-span production requests. Browser-prove Origin, native Range, expiry, revocation, decode, seek alignment, and cleanup. Never fall back to fixture prose or add semantic review, follow-up, listening, culture/reference, learner persistence, or export without separate producers/contracts |
...
| `run.production.language-explanations` | **Selected language explanation** | The client can request a fixed ordered set of five typed facets for one verified caption span and read only cold-audited private explanation artifacts. A strict adapter maps artifacts into the shared presentation contract with exact caption/span identity, host-receipted execution authority, semantically `not_reviewed` state, and closed loading/available/partial/withheld/unavailable/failed/retry states | Production caption results show the media-playback prerequisite as unavailable and expose no request control. They never fall back to design-fixture prose. Never display verified correctness, evidence support, confidence, grading, follow-up, listening, culture/reference, persistence, mastery, SRS, or export | B+C / real producer, audit, endpoint, strict client, adapter, controller, and unavailable product boundary; media-bound interaction pending |
