import {
  ...
  type LanguageJobContext,
  type ProductionAnalysisRequest,
  type ProductionSourceSession,
...
function languageContext(value: unknown, context: string, path: string): asserts value is LanguageJobContext {
  const item = object(value, context, path);
  exact(
    item,
  ...
  if (mode === "declared" && (languages.length !== 1 || reason !== null)) {
    fail(context, `${path}.languagePair.requestedSource`, "declared source requires exactly one language and no reason");
  }
  if (mode === "mixed" && (languages.length < 2 || reason !== null)) {
    fail(context, `${path}.languagePair.requestedSource`, "mixed source requires at least two languages and no reason");
  }
  if ((mode === "automatic" || mode === "unknown") && (languages.length !== 0 || reason !== null)) {
    fail(context, `${path}.languagePair.requestedSource`, `${mode} source cannot carry languages or a reason`);
  }
  if (mode === "withheld" && (languages.length !== 0 || reason === null)) {
    fail(context, `${path}.languagePair.requestedSource`, "withheld source requires only a reason");
  }
  ...
  evidence.forEach((id, index) => contentId(id, context, `${path}.detectedLanguageEvidenceContentIds[${index}]`));
}
...
export function assertProductionSourceSession(
  ...
  evidence.forEach((id, index) => contentId(id, context, `session.detectedLanguageEvidenceContentIds[${index}]`));
  if (preflight.schema !== "studio.preflight-bundle.v3" && evidence.length !== 0) {
    fail(context, "session.detectedLanguageEvidenceContentIds", "requires a V3 preflight bundle");
  }
}
...
export function assertProductionAnalysisRequest(
  ...
  exact(
    item,
    ["schema", "requestId", "sourceSessionId", "sourceRevisionId", "sourceContentId", "range", "language", "outputDepth", "options"],
    context,
    "request",
  );
  ...
  exact(
    options,
    ["speechScope", "includeLyrics", "speaker", "honorifics", "translationStyle", "captionDensity", "slowAnalysis"],
    context,
    "request.options",
  );
  ...
  oneOf(options.honorifics, new Set(["preserve", "naturalize"]), context, "request.options.honorifics");
  oneOf(options.translationStyle, new Set(["literal", "natural"]), context, "request.options.translationStyle");
  oneOf(options.captionDensity, new Set(["compact", "balanced", "relaxed"]), context, "request.options.captionDensity");
