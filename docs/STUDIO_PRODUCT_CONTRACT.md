...
- The centered welcome keeps **Preview a YouTube link with recorded demo** as its persistent bottom
  action. A collapsed 40px chooser aligned with the top-left mark contains three alternate paths:
...
| `source.input.open` | Bottom **Preview a YouTube link with recorded demo** bar | Opens and focuses the recorded-preview URL entry control; it never calls local ingest and remains outside the alternate-source chooser | Client state | Keep recorded preview separate from live local ingest; no job starts | A / recorded preview entry |
...
| `source.url` | **Paste a YouTube link for recorded preview** | Validates the URL client-side, then requests and validates a provider-metadata receipt | `presentSource()`, `resolveRemoteSource()` | Feed only the explicit recorded-preview path; metadata resolution is not media download or processing | A / recorded preview entry |
...
| `source.submit` | Arrow, labelled **Resolve metadata for recorded preview** | For a valid YouTube URL, creates a metadata-only preview session and later starts `run-006` replay when explicitly confirmed | `sourceResolution.ts`, `previewSession.ts`, recorded bundle | Preserve the recorded label and not-processed warning; never call live ingest from this action | A / recorded replay |
...
| `result.viewer.shared` | **Recorded demo** or **Your clip** viewer | Uses one result composition for media chrome, Split/Cinema/Full screen modes, learning panel, Moments overlay, and the Customize learning face behind the learning panel's **Tune** disclosure. The recorded surface renders the shell in its **workbench frame** inside a focus-panel workspace: no authority bar above the composition — the run's title heads the environment, the evidence class reads in the hero facts ("Evidence — Recorded demo") and the Source panel, and `data-result-authority` stays machine-readable on the region. The workspace's command baseline carries the learning toggles (**Saved** / **Tune**, portalled down by the workspace they control), the **Source** and **Coverage** disclosures, and the **Close** control; the transcript renders as a bare feed with a slim mono header row. The production surface keeps the standard frame with its always-on badge and inline toggles. Authority-specific adapters supply media, playback, and learning projections | Keep the visible result consistent while the stated evidence class and projection authority preserve recorded versus production meaning on every frame | A+B / shared presentation with separate authority adapters |
| `result.process-view` | Result workspace open/close over the persistent canvas | On a completed recorded run the swarm canvas never unmounts: a golden **Result** node carrying the run's language pair forms at the topology's terminus — on the orchestrator's free face, where its delivery wire can never cross another wire — and the result workspace auto-opens full-bleed over the canvas. **Close** (or Esc) closes the workspace to reveal the completed graph with the orb, framed by a passive "Recorded evidence · completed process graph" chip; the orb is the sole way back in — there is no Result/Process switch on the recorded surface. The owned-source production surface keeps its one segmented control between the validated processing canvas and the result, with Result the default once a verified caption result exists | The result stays the default and the first payoff is instant — the orb is a re-entry anchor, never a gate. The artifact node is a projection of the run's real receipted captions: it emits no trace, mounts only once the fold is complete, and its orbiting hairline aura is the one piece of ongoing motion on the settled canvas (removed under reduced motion). Closing pauses playback, issues no runtime request, and keeps the mounted result session state (playback position, viewer mode, pinned explanation, saved items, learning prep). The open graph is presentation of already-validated state, never live or resumed execution | A+B / presentation state over recorded projection and validated production projection |
