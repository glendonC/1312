...
- Selected-language explanation is a separate private post-caption Apply producer. Its closed POST
  ...
  Active or completed requests cannot execute twice. The strict browser client is landed, while
  Results wiring remains a separate UI slice.
...
| 11a. Explain selected language | Exact caption moment, selected span, prepared explanation facets, availability, and limitations | The authenticated host POST/GET, typed artifact/receipt, replaceable unavailable/test/OpenAI executor seam, cold audit, and strict client parser are implemented. Default execution is unavailable until an operator explicitly selects and enables a model. The in-player UI adapter is not wired in this runtime slice | B+C / real private producer seam; UI projection pending | Wire only the audited artifact into Results; add semantic review, follow-up, listening, culture/reference, learner state, or export only through separate producers/contracts |
...
| `run.production.language-explanations` | **Selected language explanation** | The client can request exact typed facets for one verified caption span and read only cold-audited private explanation artifacts. Available prose remains host-receipted and semantically `not_reviewed`; missing facets retain closed unavailable/withheld reasons | The in-player UI is pending. Never merge design-fixture prose with production authority or display verified correctness, evidence support, confidence, grading, follow-up, listening, culture/reference, persistence, mastery, SRS, or export | B+C / real producer, audit, endpoint, and strict client seam; UI projection pending |
