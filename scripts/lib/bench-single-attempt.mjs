...
async function validators() {
  if (!validatorsPromise) {
    validatorsPromise = (async () => {
      ...
      const [input, charge, attribution, capture] = await Promise.all([
...
export async function runSingleAttempt(
  ...
  if (typeof executor !== "function") fail("executor must be a function");
  const run = runId(runValue);
  ...
  const sourceRecordedPath = repositoryPath(sourcePath, "source input path");
  const sourceAbsolutePath = resolve(workspaceRoot, sourceRecordedPath);
  const sourceBinding = await fileReceipt(sourceAbsolutePath, sourceRecordedPath);
  ...
  exactKeys(output, ["capture"], "executor output");
  await schemaCheck(output.capture, "capture", "executor capture");
  validateCaptureForInvocation(output.capture, {
    registration,
  ...
  await writeOnceJson(resolve(workspaceRoot, paths.capture), output.capture, "capture");
...
export async function auditSingleAttemptCharges({ workspaceRoot = process.cwd() } = {}) {
