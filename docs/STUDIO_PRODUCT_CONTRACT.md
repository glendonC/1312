...
- A local runtime-start host now exists under
  ...
  development-only owned-byte and private YouTube-range ingest/probe/seal/status endpoints,
...
| 11b. Prepare learning notes and watch with tappable marks | Optional armed lenses and temperature, one result composition, per-line prepared notes, honest silence | Recorded and production results share one viewer, learning workspace, and Learning notes face. Recorded run-006 opens with an explicitly labelled `design_fixture` projection bound to its recorded caption identity, with null production and execution authority. Changing its controls requires a new session-local fixture projection. Production keeps the separate explicit action that posts `studio.learning-fine-tune.v1`; the host stores immutable `studio.learning-prep.artifact.v1`/receipt lineage and the strict client re-hashes both before projection. Both paths surface prepared help only as tappable marks: each available note is marked on its own transcript line, and in the watch room a small named mark rides the video while the playhead is inside the note's window. A mark opens its note on demand; nothing pops on its own, and silence stays a valid state. On the faced recorded workspace the on-video marks render only in the watch face; the report's clip is a preview | A+B / recorded design fixture plus real private producer, shared composition, strict adapters, controller, face, and note-mark projection | Recorded fixture prose is not runtime output or production evidence. Production notes stay host-receipted `not_reviewed` caption-context inference with empty external citations. Never present either as verified culture/history, alignment evidence, salience truth, a course, or a quiz gate. Temperature never coerces availability. No learner persistence, export, or Learning OS |
| 11c. Translate selected span | Exact caption moment, selected span, one bounded span translation, availability, and limitations | The recorded demo offers no Translate action at all; its selection bar carries only Explain and Save, and no model is called. On a live production caption result with valid private playback, the floating selection bar's Translate issues one explicit request that binds the exact verified caption identity and Unicode code-point span; the host stores immutable `studio.span-translation.artifact.v1`/receipt lineage over one explicitly configured executor (a loopback-only local Ollama model or an OpenAI opt-in) on one shared prompt contract, and the strict client re-hashes both before projection. The readout shows loading, the one host-derived translation into the opposite caption language, withheld decline, failed with bounded explicit retry, or non-retryable unavailable when no executor is configured; every missing state is visible beside the line's own tagged translation. Stale selections invalidate responses and concurrent identical requests coalesce | B+C / real private producer only, absent from the recorded demo, exact-span interaction, adapter, controller, and strict client | Keep this one bounded not_reviewed gloss of the exact selection. Never add fixture fallback, semantic review, dictionary claims, batch translation, learner persistence, or export without separate producers/contracts |
| 12. Inspect real run | Operator can audit actual tasks, operations, usage, handoffs, and failures | The owned-source status surface projects source/evidence/assessment/decision artifacts, tasks, spawns, workers, grants, media operations, evidence reads/assessments/decisions, initial-coverage failure classifications and recovery lineage, intake, human review/revocation lineage, caption job/artifact identities and counts, output lineage, and reports. Recovery facts preserve failed attempt 0 beside replacement attempt 1 and show only validated authorized, replacement-reported, or exhausted state. Separate receipt regions appear only after the host reopens and validates stored bytes and complete lineage; withheld, unavailable, reject, and revoke reasons remain visible. Artifact references link only to identities rendered in that projection. The fuller local journal remains manually loadable in `/studio/runtime/` | B / bounded product projection, stored-receipt audits, plus full inspector | Expand only with producer-backed facts; do not deep-link into unrelated inspector state |
...
| `local.source.youtube-ingest` | Canonical YouTube URL, bounded start/end, explicit local-processing confirmation | Default Studio posts the authenticated request; the host downloads at most 120 seconds through yt-dlp/ffmpeg, probes and seals exact private bytes, and hot-registers a `youtube_local` source. It accepts no destination path and grants no redistribution/publication authority | `ProductLocalRuntime` + `POST /v1/youtube-local-ingests` | Preserve the exact provider/source receipts and recorded-path separation | B / real and wired development path |
| `local.source.youtube-progress` | **queued**, **resolving**, **downloading**, **probing**, **sealing**, **registered**, or **failed** | Polls the real producer-backed job; terminal registration is cross-checked against the source list and selected only when `sourceKind` is `youtube_local` | `GET /v1/youtube-local-ingests/:ingestId` | No fabricated percentages, automatic retry, recorded fallback, or path disclosure | B / real and wired |
| `local.source.select` | **Registered owned source** or **Registered YouTube local source** | Selects a stable source-session/revision from the authority-specific filtered host list; a newly ingested source is cross-checked and auto-selected | `RuntimeSourceRegistry` + strict client parser | Preserve stale-revision and source-authority checks | B / real and wired |
| `local.source.facts` | Source kind, receipt scope, measured duration/tracks, sealed preflight, language-evidence availability, content/session/revision ids | Projects the validated registered source summary; no filename/path/provider wire field is returned | Host source registry over provider-neutral local source sessions | Expand only with validated source-adapter facts | B / real and wired |
...
| `run.production.tasks` | **Production tasks** | Shows scheduler-owned identity, objective, owner, parent, dependencies, required outputs, journal status, and reviewed-memory binding identity when present on the task job context | Keep this production-only; absence is unavailable until `task.created` is validated; reviewed memory stays unavailable without a durable consumption receipt | B / real and wired bounded facts |
| `run.production.reviewed-memory` | **Reviewed memory consumption** | Shows host-injected consumption receipt identity, materialization and snapshot content ids, fail-closed policy, and bound entry keys from validated task job contexts | Empty/unavailable until `task.created` and a durable consumption binding exist; materialization alone never invents consumption; not a score win or glossary quality claim | B / real host consumption binding and product projection |
...
| `run.production.agent-recovery` | **Initial-coverage recovery** | Shows validated executor failure classifications, one scheduler authorization, failed attempt 0, distinct replacement attempt 1, reserved allocation ceilings, and only a validated `replacement_reported` or `exhausted` terminal receipt | Empty and classified-only states remain explicit. Classification alone does not authorize replacement. Exhaustion shows zero remaining attempts, no replacement report, and withheld root. This panel provides no attempt 2, best-of-K selection, quality retry, forecast, or root/caption/Learning recovery claim | B / real host receipts, strict adapter, coordination summary, and product projection |
...
| `run.production.operations` | **Production operations** | Shows validated `media.operation_started` identity, capability, task/worker/grant binding, input track and requested range, plus output/receipt or failure only when the corresponding terminal event exists. The default deterministic proof now produces one completed `media.seek` card from real ffmpeg host events | Empty remains empty when no operation event exists; a plan, grant, or worker statement never creates an operation card. The media panel states which operation classes it projects, and the absence of an unprojected class is explicitly not evidence about whether it ran | B / real child bridge, media host, adapter, and product projection for the bounded seek proof |
| `run.production.semantic-evidence` | **Semantic media evidence operations** | Shows validated `semantic.evidence_started` identity, visible completion-audit state, executor task/worker/execution/launch/grant binding, exact source artifact/content/track and requested range, and, only after the matching terminal event, returned range, producer/runtime/configuration lineage, output artifact/content, receipt/content, observation count, and availability state with truncation | Empty remains empty until a semantic evidence event is validated; a plan, grant, worker claim, role label, or animation never creates a card. Observation counts and availability are receipted output identity, not transcription accuracy, translation quality, meaning, or publication. An `absent_or_invalid` reopen withholds outputs and stays visible | B / real recognizer host events, adapter, and product projection |
...
| `run.production.learning-preps` | **Learning notes** | For the active verified caption result, the face arms lenses and one closed temperature, posts one explicit typed prep request, and projects host-derived not-requested, loading, ready, failed, or unavailable states with closed reasons. Ready summaries show per-lens surfaced or abstained outcomes, beat or watch-through segmentation, and honest all-withheld. Prepared available notes appear as tappable marks on their verified caption lines and open on demand; withheld and abstained help stays silent | An unconfigured executor is non-retryable unavailable and a real provider requires explicit host-side model opt-in; revoked caption authority blocks new prep; no fixture prose appears; mark silence is a valid state, not an error. Prep availability is receipted `not_reviewed` caption-context inference, never verified culture/history, ranking truth, or learner persistence | B / real host producer, strict client, controller, face, and note marks over verified prep artifacts |
| `run.production.span-translations` | **Selected span translation** | With exact production playback, the selection bar's Translate creates one explicit bounded request for the exact Unicode code-point span and projects host-derived loading, translated, withheld, failed, or unavailable states with closed reasons. The translated readout is one receipted `not_reviewed` gloss into the opposite caption language; the line's verified translation stays revealed beside it under its own tag | No automatic request, caller prompt, fixture prose, or dictionary and semantic-quality claim is accepted. An unconfigured executor is non-retryable unavailable; a real model requires explicit host-side opt-in flags, either a loopback-only local Ollama endpoint or the OpenAI key the caption producer already demands; revoked caption authority blocks new translations; retries stop at the fixed ceiling | B+C / real producer, audit, endpoint, strict client, adapter, controller, and exact-span interaction |
...
| `agent.workbench` | **Workbench** | Keeps recorded visual source evidence beside current state, role work, and recent actions; the recorded source plays in the same squircle screen and on-video chrome as the result viewer, with the below-frame transport reserved for pictureless media | Pair an agent-scoped evidence environment with validated current operations and reportable decisions | A / recorded replay |
...
| `result.caption-view` | **1321 / Cold / Diff** | Deliberately absent: the result surface renders no cold/diff comparison (`Results.tsx` states the non-claim); recorded baselines remain in `captions.json` | Comparison only when both paths have compatible recorded evidence | C / missing |
...
| `result.method` | **Method** disclosure | At evidence depth the report's command baseline holds one consolidated technical record beside Source, projected from typed bundle fields through `projectTechnicalRecord` and `projectResultAccounting`: recorded wall clock and first-committed-line latency, manifest worker count with an explicit replay-not-rerun note, gate checks/failures with recorded gate names, cross-recogniser corroboration counts including honest unmeasurable lines, the per-line captioned/withheld/silent accounting, probe content identity/bytes/tracks, the ingest source window, the score artifact's accuracy state (not scored without gold; a recorded delta only when one exists), the glossary/correction rows the run feeds back with their promotion disposition, and the run file links with the evidence index's hashed-artifact count. There is no separate Coverage disclosure; at captions depth the baseline holds Source alone and the accounting stays in the hero and brief | Keep benchmark accuracy in the bench lane: the panel states scoring status honestly, never converts coverage or latency into a quality claim, and presents worker labels as manifest facts, not reconstructed lineage. Values come from typed fields, never parsed prose notes | A / recorded replay |
| `result.evidence` | Recorded evidence index | The index loads and validates with the bundle; the Method disclosure states its hashed-artifact count and links `evidence.json`, and no in-panel decision browser renders terminal cue decisions | Link production artifacts/operations without recasting legacy prose as provenance | A / recorded replay |
...
| `result.viewer.shared` | **Recorded demo** or **Your clip** viewer | Uses one result composition for the media chrome and one learning experience. In the recorded surface's watch room that experience is a command-driven stage: a single bottom command bar (Transcript, **Details** folding Source and Coverage into one plain-language read, Saved, and the Learning notes face under **Notes**) drives one reusable docked panel, so every finished-run disclosure is a mode of that one panel rather than a scattered rail, dossier, or dialog. The recorded surface renders the shell in its **workbench frame** inside the faced workspace: no authority bar above the composition — the run's title heads the report's environment, the hero facts carry only the run's accounting (languages, range, per-line counts), the evidence class is stated by the Source disclosure ("recorded evidence: an honest demo replay") and the preflight facts, and `data-result-authority` stays machine-readable on the region. The workbench viewer carries no Split/Cinema choice — the watch face is the stage and browser Full screen is the one mode control; the room sizes the docked panel and there is no width control. The report keeps the focus-panel command baseline — the **Source** and, at evidence depth, **Method** disclosures beside the one exit to the completed graph — while the watch room is commanded from its bottom bar and exits through a top-right **Back** that steps to the report. The panel opens closed: the room is first a normal video, with bilingual captions burned on the picture and their one control riding it (Both, one language, or Listen, a fill-in practice that deterministically blanks words which reveal on tap), the current line lit in the transcript at line level, and recorded speaker chips where the clip carries diarization. Selecting a bar option docks that content; selecting it again, or Close, empties the panel so the centered clip takes the whole room. Selecting caption text, on the video or in the transcript, raises one floating selection bar (Explain, Save; Translate appears only on live production runs where a receipted span translation is real), a one-line hint above the command bar names that gesture, and the run's gold identity mark anchors the room's top-left. The production surface keeps the standard frame with its always-on badge, Split/Cinema/Full screen modes, and inline toggles. Authority-specific adapters supply media, playback, and learning projections | Keep the visible result consistent while the stated evidence class and projection authority preserve recorded versus production meaning on every frame | A+B / shared presentation with separate authority adapters |
| `result.process-view` | Result workspace faces over the persistent canvas | On a completed recorded run the swarm canvas never unmounts: a golden **Result** node carrying the run's language pair forms at the topology's terminus — on the orchestrator's free face, where its delivery wire can never cross another wire — and the result workspace auto-opens full-bleed over the canvas on its **arrival** face: a once-per-run completion statement ("Your video has finished processing.") over the result brief, with **View result** (or Esc) continuing to the **report** face — clip preview beside the sentence-form brief. **Watch & study** enters the **watch** face, a full-viewport room of clip beside transcript; **Back** (or Esc) steps to the report, and **Close** (or Esc from the report) reveals the completed graph with the orb and the run's persistent bottom bar in its terminal state (Done, **Open Results**, **Clear**); the orb and Open Results resume the last face, never arrival, and there is no Result/Process switch on the recorded surface. The owned-source production surface keeps its one segmented control between the validated processing canvas and the result, with Result the default once a verified caption result exists | The result stays the default and the first payoff is instant — the orb is a re-entry anchor, never a gate. The artifact node is a projection of the run's real receipted captions: it emits no trace, mounts only once the fold is complete, and its orbiting hairline aura is the one piece of ongoing motion on the settled canvas (removed under reduced motion). Esc steps back one room at a time. Closing pauses playback, issues no runtime request, and keeps the mounted result session state (playback position, prep, pinned explanation, saved items) across every face. The open graph is presentation of already-validated state, never live or resumed execution | A+B / presentation state over recorded projection and validated production projection |
| `result.brief` | Arrival statement and the report's **Result breakdown** | Renders the finished run as sentences whose parameter values — range, languages, source title and attribution, per-line counts — are read from the bundle through the shared accounting projection and tinted with the preparation-stage palettes (source coral, range citron, language blue, coverage lilac), so the completion answers in the colors of the request | `projectResultAccounting(bundle)`, `run.clip.source` | The brief owns no numbers or names of its own; a missing licence drops the clause instead of inventing one | A / recorded projection |
| `result.recorded-learning-prep` | **Recorded learning notes** | Opens run-006 with prepared grammar and situating notes derived only from its bound `studio.learning-prototype-fixture.v1`, marked on their transcript lines. Changing the depth clears them until **Prepare notes for this clip** is selected | Demonstrate the shared learning result without claiming a runtime producer, receipt, semantic review, or production authority | A / recorded `design_fixture`, session-local interaction |
...
| `result.raw` | Run artifact links | No standalone Raw run disclosure exists; the run's declared files, pack link, and evidence index link render inside the Method disclosure at evidence depth | Keep raw recorded and production protocols distinct | A / recorded replay |
...
| Produce learning prep | One private immutable `studio.learning-prep.artifact.v1` plus receipt bound to one exact verified caption result and one typed `studio.learning-fine-tune.v1`; host-derived beat or watch-through segmentation; per-moment candidates and lens outcomes with closed abstention reasons | Accept only the exact caption identity plus closed fine-tune; reject caller beats, candidates, prompts, prose, paths, and open fields; unconfigured executor and revoked authority fail closed; temperature never turns withheld or unavailable help into available help |
...
registered provider-neutral local source sessions and one bounded child. The default product path can
create a registered V1 source from either explicitly attested browser-selected owned bytes or an
explicitly confirmed bounded YouTube-local range. Implemented glue:
...
- A parallel YouTube URL/range/local-processing confirmation path using the same host connection,
  strict status parser, source list cross-check, staged forecast/start flow, and no replay identity
...
- Treat a learning-prep artifact, receipt, or prepared note as verified culture/history,
  alignment evidence, semantic quality, salience-ranking truth, course structure, a watch gate,
  learner persistence, mastery, SRS, or export eligibility; mark silence and all-withheld prep
  remain honest states
...
| Learning-prep producer, cold audit, strict client, controller, face, and note marks | `src/studio/runtime/production/learningPrep/`, `src/studio/runtime/production/model/learningPrep.ts`, `src/studio/localRuntime/client/learningPrepResponses.ts`, `src/studio/localRuntime/productionLearningPrepController.ts`, `src/studio/learning/LearningFineTuneFace.tsx`, `src/studio/learning/momentContent.tsx`, `tests/studio-learning-prep.test.ts` |
