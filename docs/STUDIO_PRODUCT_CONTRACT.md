...
| 11a. Explain selected language | Exact caption moment, selected span, prepared explanation facets, availability, and limitations | Recorded Results explicitly bind `studio.learning-prototype-fixture.v1` through a prototype adapter. Production caption results use the same presentation tree through a verified-caption adapter. Exactly one decoded private player is available only when its disposable handle closes over the active runtime, source revision, source artifact, source content, caption job, caption artifact, caption content, unrevoked authority, and source-zero timeline. It supports play, pause, current-time cue activation, and exact line seek. Replacement, expiry, load failure, or unmount clears the media element and revokes the handle. Browser proof covers exact Origin, native Range, decode, seek alignment, playback advance, and cleanup. Production selection and explanation requests remain unavailable, expose no My Set control, and never fall back to fixture prose. A separate strict production adapter and controller map only meaning, word, phrase, grammar, and translation-choice artifacts across loading, available, partial, withheld, unavailable, failed, and explicit retry states. Default execution remains unavailable until an operator explicitly selects and enables a model | A+B+C / explicit design-fixture mode plus real private producer, adapter, controller, content-bound private playback, verified media transport, strict client, and unavailable explanation-interaction boundary | Wire explicit exact-span production requests from the verified playback moment. Never fall back to fixture prose or add semantic review, follow-up, listening, culture/reference, learner persistence, or export without separate producers/contracts |
...
| `run.production.language-explanations` | **Selected language explanation** | The client can request a fixed ordered set of five typed facets for one verified caption span and read only cold-audited private explanation artifacts. A strict adapter maps artifacts into the shared presentation contract with exact caption/span identity, host-receipted execution authority, semantically `not_reviewed` state, and closed loading/available/partial/withheld/unavailable/failed/retry states | Production caption results now bind one decoded private player to the exact active verified caption and source-zero timeline. Explanation selection and request controls remain unavailable and never fall back to design-fixture prose. Never display verified correctness, evidence support, confidence, grading, follow-up, listening, culture/reference, persistence, mastery, SRS, or export | B+C / real producer, audit, endpoint, strict client, adapter, controller, and content-bound private playback; exact-span interaction pending |
