...
export function orchestratorPrompt(task: TaskRecord): string {
  ...
  const exactTools = restudied
    ? ["task_spawn_request", "task_reports_wait", "report_disposition", "artifact_read", "study_restudy_request", ...(separationEnabled ? ["study_separation_request"] : []), "study_synthesize"]
    : generalized
